...
then
   echo "cp mode"
   echo "cp $dir_name/clip0.$file_format $destdir/\"\`$output\`_cut_$file_name\"" >>run.sh
else
   echo "concat mode"
   echo "ffmpeg -f concat -i $dir_name/concat.txt -c copy $destdir/\"\`$output\`_cut_$file_name\"" >>run.sh
fi
